...
class MoleculeModel(nn.Module):
    ...
    def forward(self,
        ...
        """
        Forward pass through encoder and FFN, returning intermediate activations **after each non-linearity**
        (e.g., after ReLU), as required for continual learning algorithms like GNT.
    
        :return: (final_output, list of activations after each non-linearity)
        """
        # Step 1: get molecular embedding from encoder (MPN)
        x = self.encoder(batch, features_batch, atom_descriptors_batch,
                         atom_features_batch, bond_features_batch)
    
        hidden_activations = []  # store activations after each non-linearity (e.g., ReLU)
    
        # Step 2: manually forward through FFN, saving after activations
        for layer in self.ffn:
            x = layer(x)
    
            # Record activation output (e.g., after nn.ReLU, nn.SiLU, etc.)
            if isinstance(layer, (nn.ReLU, nn.LeakyReLU, nn.PReLU, nn.Tanh, nn.SELU, nn.ELU)):
                hidden_activations.append(x)
    
        # Step 3: handle task-specific output transforms
        out = x
        ...
        if self.loss_function == 'mve':
            means, variances = torch.split(out, out.shape[1] // 2, dim=1)
            variances = self.softplus(variances)
            out = torch.cat([means, variances], axis=1)
        if self.loss_function == 'evidential':
            means, lambdas, alphas, betas = torch.split(out, out.shape[1] // 4, dim=1)
            lambdas = self.softplus(lambdas)
            alphas = self.softplus(alphas) + 1
            betas = self.softplus(betas)
            out = torch.cat([means, lambdas, alphas, betas], dim=1)
        if self.loss_function == 'dirichlet':
            out = nn.functional.softplus(out) + 1
