...
class GnT(object):
    ...
    def __init__(
        ...
        self.device = device
        self.net = net
        self.num_hidden_layers = int(len(self.net)//3)
        self.loss_func = loss_func
        self.accumulate = accumulate
        self.total_replaced_neurons_this_epoch = 0
        ...
        """
        self.util = [torch.zeros(self.net[i * 3 + 1].out_features).to(self.device) for i in range(self.num_hidden_layers)]
        self.bias_corrected_util = \
            [torch.zeros(self.net[i * 3 + 1].out_features).to(self.device) for i in range(self.num_hidden_layers)]
        self.ages = [torch.zeros(self.net[i * 3 + 1].out_features).to(self.device) for i in range(self.num_hidden_layers)]
        self.m = torch.nn.Softmax(dim=1)
        self.mean_feature_act = [torch.zeros(self.net[i * 3 + 1].out_features).to(self.device) for i in range(self.num_hidden_layers)]
    ...
    def compute_bounds(self, hidden_activation, init='kaiming'):
        ...
        if init == 'default':
            bounds = [sqrt(1 / self.net[i * 3 + 1].in_features) for i in range(self.num_hidden_layers)]
        elif init == 'xavier':
            bounds = [torch.nn.init.calculate_gain(nonlinearity=hidden_activation) *
                      sqrt(6 / (self.net[i * 3 + 1].in_features + self.net[i * 3 + 1].out_features)) for i in
                      range(self.num_hidden_layers)]
        elif init == 'lecun':
            bounds = [sqrt(3 / self.net[i * 3 + 1].in_features) for i in range(self.num_hidden_layers)]
        else:
            bounds = [torch.nn.init.calculate_gain(nonlinearity=hidden_activation) *
                      sqrt(3 / self.net[i * 3 + 1].in_features) for i in range(self.num_hidden_layers)]
        bounds.append(1 * sqrt(3 / self.net[self.num_hidden_layers * 3 + 1].in_features))
        return bounds
    ...
    def update_utility(self, layer_idx=0, features=None, next_features=None):
        with torch.no_grad():
            ...
            current_layer = self.net[layer_idx * 3 + 1]
            next_layer = self.net[layer_idx * 3 + 4]
            ...
            if self.util_type == 'weight':
                ...
                input_wight_mag = self.net[layer_idx*3 +1].weight.data.abs().mean(dim=1)
        ...
        with torch.no_grad():
            for i in range(self.num_hidden_layers):
                if num_features_to_replace[i] == 0:
                    continue
                current_layer = self.net[i * 3 + 1]
                next_layer = self.net[i * 3 + 4]
    ...
    def update_optim_params(self, features_to_replace, num_features_to_replace):
        ...
        if self.opt_type == 'adam':
            for i in range(self.num_hidden_layers):
                ...
                if num_features_to_replace[i] == 0:
                    continue
                self.opt.state[self.net[i * 3 + 1].weight]['exp_avg'][features_to_replace[i], :] = 0.0
                self.opt.state[self.net[i * 3 + 1].bias]['exp_avg'][features_to_replace[i]] = 0.0
                self.opt.state[self.net[i * 3 + 1].weight]['exp_avg_sq'][features_to_replace[i], :] = 0.0
                self.opt.state[self.net[i * 3 + 1].bias]['exp_avg_sq'][features_to_replace[i]] = 0.0
                self.opt.state[self.net[i * 3 + 1].weight]['step'][features_to_replace[i], :] = 0
                self.opt.state[self.net[i * 3 + 1].bias]['step'][features_to_replace[i]] = 0
                # output weights
                self.opt.state[self.net[i * 3 + 4].weight]['exp_avg'][:, features_to_replace[i]] = 0.0
                self.opt.state[self.net[i * 3 + 4].weight]['exp_avg_sq'][:, features_to_replace[i]] = 0.0
                self.opt.state[self.net[i * 3 + 4].weight]['step'][:, features_to_replace[i]] = 0
